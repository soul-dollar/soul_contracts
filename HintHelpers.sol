        ...
        while (currentTroveuser != address(0) && remainingLUSD > 0 && _maxIterations-- > 0) {
            ...
            if (netLUSDDebt > remainingLUSD) {
                if (netLUSDDebt > MIN_NET_DEBT) {
                    uint maxRedeemableLUSD = LiquityMath._min(remainingLUSD, netLUSDDebt - MIN_NET_DEBT);
                    ...
                    remainingLUSD = remainingLUSD - maxRedeemableLUSD;
                }
                break;
            } else {
                remainingLUSD = remainingLUSD - netLUSDDebt;
            }
